...
class TestObsidianWriter:
    ...
    def test_write_includes_tags(
        ...
        """Tags should be included in frontmatter (generated by graph enricher)."""
        output_path = writer.write(sample_bookmark, sample_result)
        content = output_path.read_text()

        assert "tags:" in content
        # Graph enricher generates hierarchical tags
        assert "  - source/twitter" in content
        assert "  - twitter/tweet" in content
        assert "  - person/testuser" in content
        # Content mentions "python" so topic/python should match
        assert "  - topic/python" in content

    def test_write_always_has_base_tags(
        self,
        writer: ObsidianWriter,
        sample_bookmark: Bookmark,
    ):
        """Graph enricher always generates base tags (source, type, person)."""
        result = ProcessResult(
            success=True,
            title="No tags tweet",
        ...
        # Enricher always adds base tags regardless of ProcessResult.tags
        assert "tags:" in content
        assert "  - source/twitter" in content
        assert "  - twitter/tweet" in content
        assert "  - person/testuser" in content
